...
import 'package:flutter_game/elements.dart';
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Size screenSize = Size.zero,
      ...
      platformSize = Size(200, 50),
      brickSize = Size(70, 25);
  ...
  brickFilter() {
    for (int x = 0; x < bricks.length; x++) {
      ...
      if (brick.rect
          .contains(Offset(playBall.position.x, playBall.position.y))) {
  ...
  createBrickWall() {
    ...
    for (int x = 0; x < maxRowCount; x++) {
      ...
      double leftOffset =
          ((screenSize.width - (currentColumnCount * brickSize.width)) / 2);
      for (int y = 0; y < currentColumnCount; y++) {
        Rect rect = Rect.fromLTWH(leftOffset + (y.toDouble() * brickSize.width),
            x * brickSize.height, brickSize.width, brickSize.height);
          ...
          child: GestureDetector(
            onTap: () {
              if (!launched && !finishing) {
                launched = true;
                playBall.position = PVector(pointer.dx,
                    screenSize.height - platformSize.height - playBall.radius);
